...
module namespace model="http://www.tei-c.org/pm/models/prohd/print";
            ...
            return
                    ...
                    case element(hi) return
                        if (@rendition="#u" and @hand="#unknown") then
                            (
                                ...
                                return
                                                                printcss:note(map:merge(($config, map:entry("template", true()))), ., ("tei-hi2", css:map-rend-to-class(.)), $content, (), ())
                            )
                    ...
                    case element(subst) return
                        if ($parameters?mode='norm' and del and add) then
                            printcss:alternate($config, ., ("tei-subst", "choice", css:map-rend-to-class(.)), ., add[1], del[1])
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(seg) return
                        if (@type="comment") then
                            (
                                ...
                                return
                                                                printcss:note(map:merge(($config, map:entry("template", true()))), ., ("tei-seg2", css:map-rend-to-class(.)), $content, (), ())
                            )
                        ...
                        if (ancestor::profileDesc and matches(., 'N/A')) then
                            ...
                            if (text()) then
                                ...
                                if (@when and not(text())) then
                                    html:inline($config, ., ("tei-date3", css:map-rend-to-class(.)), @when)
                                else
                                    if (ancestor::text and @when) then
                                        printcss:alternate($config, ., ("tei-date4", css:map-rend-to-class(.)), ., ., @when)
                                    else
                                        if (text()) then
