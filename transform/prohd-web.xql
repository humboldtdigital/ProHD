            ...
            return
                    ...
                    case element(teiHeader) return
                        if ($parameters?header='short') then
                            ...
                            if ($parameters?display='commentary') then
                                ...
                                return
                                                                html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-teiHeader4", css:map-rend-to-class(.)), $content)                                => model:map($node, $trackIds)
                            else
                    ...
                    case element(pb) return
                        if (@facs) then
                            ...
                            return
                                                        html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-pb1", "lb", css:map-rend-to-class(.)), $content)                            => model:map($node, $trackIds)
                        else
                            if (@facs) then
                                html:webcomponent($config, ., ("tei-pb2", "facs", "lb", css:map-rend-to-class(.)), @n, 'pb-facs-link', map {"facs": replace(@facs, '^img:(.*)$', '$1')})                                => model:map($node, $trackIds)
                            else
                                if (@n and $parameters?view='page') then
                                    html:inline($config, ., ("tei-pb3", css:map-rend-to-class(.)), @n)                                    => model:map($node, $trackIds)
                                else
                                    if (@n) then
                                        html:inline($config, ., ("tei-pb4", css:map-rend-to-class(.)), @n)                                        => model:map($node, $trackIds)
                                    else
                                        $config?apply($config, ./node())
                    ...
                    case element(choice) return
                        if (abbr and expan) then
                            (
                                html:alternate($config, ., ("tei-choice1", "choice", css:map-rend-to-class(.)), ., expan[1], abbr[1], map {})                                => model:map($node, $trackIds),
                                html:alternate($config, ., ("tei-choice2", "choice-alternate", css:map-rend-to-class(.)), ., abbr[1], expan[1], map {})                                => model:map($node, $trackIds)
                            )

                        else
                            if (sic and corr) then
                                (
                                    html:alternate($config, ., ("tei-choice3", "choice", css:map-rend-to-class(.)), ., corr[1], sic[1], map {})                                    => model:map($node, $trackIds),
                                    html:alternate($config, ., ("tei-choice4", "choice-alternate", css:map-rend-to-class(.)), ., sic[1], corr[1], map {})                                    => model:map($node, $trackIds)
                                )

                            else
                                if (reg and orig) then
                                    (
                                        html:alternate($config, ., ("tei-choice5", "choice", css:map-rend-to-class(.)), ., reg[1], orig[1], map {})                                        => model:map($node, $trackIds),
                                        html:alternate($config, ., ("tei-choice6", "choice-alternate", css:map-rend-to-class(.)), ., orig[1], reg[1], map {})                                        => model:map($node, $trackIds)
                                    )

                                else
                                    if ($parameters?mode='norm' and abbr and expan) then
                                        html:alternate($config, ., ("tei-choice7", "choice", css:map-rend-to-class(.)), ., expan[1], abbr[1], map {})                                        => model:map($node, $trackIds)
                                    else
                                        if ($parameters?mode='norm' and sic and corr) then
                                            html:alternate($config, ., ("tei-choice8", "choice", css:map-rend-to-class(.)), ., corr[1], sic[1], map {})                                            => model:map($node, $trackIds)
                                        else
                                            if ($parameters?mode='norm' and orig and reg) then
                                                html:alternate($config, ., ("tei-choice9", "choice", css:map-rend-to-class(.)), ., reg[1], orig[1], map {})                                                => model:map($node, $trackIds)
                                            else
                                                $config?apply($config, ./node())
                    case element(hi) return
                        if (@rendition="#u" and @hand="#unknown") then
                            (
                                ...
                                return
                                                                html:note(map:merge(($config, map:entry("template", true()))), ., ("tei-hi2", css:map-rend-to-class(.)), $content, (), ())                                => model:map($node, $trackIds)
                            )
                            ...
                            if (@rendition="#mMM") then
                                ...
                                if (@rendition="#i") then
                                    ...
                                    if (@rendition="#b") then
                                        html:inline($config, ., ("tei-hi5", css:map-rend-to-class(.)), .)                                        => model:map($node, $trackIds)
                                    else
                                        if (@rendition="#uu") then
                                            html:inline($config, ., css:get-rendition(., ("tei-hi6", css:map-rend-to-class(.))), .)                                            => model:map($node, $trackIds)
                                        else
                                            if (@rendition="#sup") then
                                                html:inline($config, ., css:get-rendition(., ("tei-hi7", css:map-rend-to-class(.))), .)                                                => model:map($node, $trackIds)
                                            else
                                                if (@rendition="#u") then
                                                    html:inline($config, ., css:get-rendition(., ("tei-hi8", css:map-rend-to-class(.))), .)                                                    => model:map($node, $trackIds)
                                                else
                                ...
                                if (parent::person or parent::place) then
                                    ...
                                    if (@place) then
                                        ...
                                        if (parent::div and not(@place)) then
                                            html:block($config, ., ("tei-note5", css:map-rend-to-class(.)), .)                                            => model:map($node, $trackIds)
                                        else
                                            if (not(@place)) then
                                                html:inline($config, ., ("tei-note6", css:map-rend-to-class(.)), .)                                                => model:map($node, $trackIds)
                                            else
                                                $config?apply($config, ./node())
                    case element(dateline) return
                        if (@rend="align(right)") then
                            html:block($config, ., ("tei-dateline1", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                        else
                            if (not(@rend)) then
                                html:block($config, ., ("tei-dateline2", css:map-rend-to-class(.)), .)                                => model:map($node, $trackIds)
                            else
                                $config?apply($config, ./node())
                    ...
                    case element(subst) return
                        if ($parameters?mode='norm' and del and add) then
                            html:alternate($config, ., ("tei-subst", "choice", css:map-rend-to-class(.)), ., add[1], del[1], map {})                            => model:map($node, $trackIds)
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(publisher) return
                        if (ancestor::teiHeader) then
                            html:inline($config, ., ("tei-publisher", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(fileDesc) return
                        if ($parameters?display='commentary') then
                            ...
                            if ($parameters?header='short') then
                                (
                                    html:link($config, ., ("tei-fileDesc2", "header-short", css:map-rend-to-class(.)), titleStmt, $parameters?doc, (), map {})                                    => model:map($node, $trackIds)
                                )

                            else
                                html:title($config, ., ("tei-fileDesc3", css:map-rend-to-class(.)), titleStmt)                                => model:map($node, $trackIds)
                    ...
                    case element(seg) return
                        if (@type="comment") then
                            (
                                ...
                                return
                                                                html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-seg1", css:map-rend-to-class(.)), $content)                                => model:map($node, $trackIds),
                                let $params := 
                                    map {
                                ...
                                return
                                                                html:note(map:merge(($config, map:entry("template", true()))), ., ("tei-seg2", css:map-rend-to-class(.)), $content, (), ())                                => model:map($node, $trackIds)
                            )

                        else
                            html:webcomponent($config, ., ("tei-seg3", css:map-rend-to-class(.)), ., 'pb-highlight', map {"key": replace(@xml:id, '^s\.(.*)$', 't.$1'), "highlight-self": 'highlight-self'})                            => model:map($node, $trackIds)
                    case element(profileDesc) return
                        if ($parameters?header='short') then
                            (
                                html:block($config, ., ("tei-profileDesc1", "header-short", css:map-rend-to-class(.)), abstract[@xml:lang="es"])                                => model:map($node, $trackIds)
                            )
                            ...
                            if ($parameters?display='commentary' and creation[count(orgName)=2]) then
                                ...
                                return
                                                                html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-profileDesc2", css:map-rend-to-class(.)), $content)                                => model:map($node, $trackIds)
                            else
                                if ($parameters?display='commentary' and creation[count(orgName)=1]) then
                                    ...
                                    return
                                                                        html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-profileDesc3", css:map-rend-to-class(.)), $content)                                    => model:map($node, $trackIds)
                                else
                                    if ($parameters?display='commentary' and correspDesc) then
                                        ...
                                        return
                                                                                html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-profileDesc4", css:map-rend-to-class(.)), $content)                                        => model:map($node, $trackIds)
                                    else
                                        if ($parameters?display='commentary' and creation[count(persName)=2]) then
                                            ...
                                            return
                                                                                        html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-profileDesc5", css:map-rend-to-class(.)), $content)                                            => model:map($node, $trackIds)
                                        else
                                            if ($parameters?display='commentary' and creation[count(persName)=1]) then
                                                ...
                                                return
                                                                                                html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-profileDesc6", css:map-rend-to-class(.)), $content)                                                => model:map($node, $trackIds)
                                            else
                                                html:omit($config, ., ("tei-profileDesc7", css:map-rend-to-class(.)), .)                                                => model:map($node, $trackIds)
                    case element(email) return
                        html:inline($config, ., ("tei-email", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    case element(text) return
                        if ($parameters?display='landing') then
                            html:body($config, ., ("tei-text1", css:map-rend-to-class(.)), let $lang := $parameters?language let $l := if ($lang = ('en', 'es', 'de')) then $lang else 'en' return root(.)//text[@xml:lang=$l])                            => model:map($node, $trackIds)
                        else
                    ...
                    case element(licence) return
                        if (@target) then
                            html:link($config, ., ("tei-licence1", "licence", css:map-rend-to-class(.)), ., @target, (), map {})                            => model:map($node, $trackIds)
                        else
                            html:omit($config, ., ("tei-licence2", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                    case element(editor) return
                        html:inline($config, ., ("tei-editor", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    ...
                    case element(g) return
                        if ($parameters?mode='norm') then
                            html:omit($config, ., ("tei-g1", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                        else
                            if (@ref) then
                                html:inline($config, ., ("tei-g2", css:map-rend-to-class(.)), .)                                => model:map($node, $trackIds)
                            else
                                if (not(text())) then
                                    html:glyph($config, ., ("tei-g3", css:map-rend-to-class(.)), .)                                    => model:map($node, $trackIds)
                                else
                                    $config?apply($config, ./node())
                    ...
                    case element(p) return
                        if (parent::physDesc) then
                            html:paragraph($config, ., ("tei-p1", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                        else
                    ...
                    case element(publicationStmt) return
                        if ($parameters?display='commentary') then
                            ...
                            return
                                                        html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-publicationStmt", css:map-rend-to-class(.)), $content)                            => model:map($node, $trackIds)
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(ref) return
                        if (@target) then
                            ...
                            if (@ana) then
                                html:inline($config, ., ("tei-ref2", "itinerary", css:map-rend-to-class(.)), .)                                => model:map($node, $trackIds)
                            else
                                if (not(@target)) then
                    ...
                    case element(bibl) return
                        if (@sameAs) then
                            html:inline($config, ., ("tei-bibl1", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                        else
                            if ($parameters?display='commentary' and not(parent::p)) then
                                ...
                                return
                                                                html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-bibl2", css:map-rend-to-class(.)), $content)                                => model:map($node, $trackIds)
                            else
                                if (parent::listBibl) then
                                    html:listItem($config, ., ("tei-bibl3", css:map-rend-to-class(.)), ., ())                                    => model:map($node, $trackIds)
                                else
                                    $config?apply($config, ./node())
                    ...
                    case element(date) return
                        if (ancestor::profileDesc and matches(., 'N/A')) then
                            ...
                            if (ancestor::text and @when) then
                                html:alternate($config, ., ("tei-date4", css:map-rend-to-class(.)), ., ., @when, map {})                                => model:map($node, $trackIds)
                            else
                                if (text()) then
                                    html:inline($config, ., ("tei-date5", css:map-rend-to-class(.)), .)                                    => model:map($node, $trackIds)
                                else
                                    $config?apply($config, ./node())
                    case element(title) return
                        if (parent::titleStmt/parent::fileDesc) then
                            (
                                if (@type="main") then
                                    html:heading($config, ., ("tei-title1", css:map-rend-to-class(.)), ., ())                                    => model:map($node, $trackIds)
                                else
                                    (),
                                if (preceding-sibling::title or @type="sub") then
                                    html:heading($config, ., ("tei-title2", css:map-rend-to-class(.)), ., 3)                                    => model:map($node, $trackIds)
                                else
                                    ()
                            )
                    ...
                    case element(titleStmt) return
                        if ($parameters?header='short') then
                            (
                                ...
                                return
                                                                html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-titleStmt4", css:map-rend-to-class(.)), $content)                                => model:map($node, $trackIds),
                                html:inline($config, ., ("tei-titleStmt5", css:map-rend-to-class(.)), author)                                => model:map($node, $trackIds),
                                (: incipit :)
                                html:block($config, ., ("tei-titleStmt6", "incipit", css:map-rend-to-class(.)), root(.)//seg[@type='incipit'])                                => model:map($node, $trackIds)
                            )
                            ...
                            if ($parameters?display='commentary' and count(editor)=2) then
                                ...
                                return
                                                                html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-titleStmt8", css:map-rend-to-class(.)), $content)                                => model:map($node, $trackIds)
                            else
                                if ($parameters?display='commentary' and count(editor)=1) then
                                    ...
                                    return
                                                                        html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-titleStmt9", css:map-rend-to-class(.)), $content)                                    => model:map($node, $trackIds)
                                else
                    ...
                    case element(gap) return
                        if (desc) then
                            ...
                            if (@quantity) then
                                html:inline($config, ., ("tei-gap2", css:map-rend-to-class(.)), .)                                => model:map($node, $trackIds)
                            else
                    ...
                    case element(persName) return
                        if (ancestor::profileDesc and matches(., 'N/A')) then
                            ...
                            return
                                                        html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-persName1", css:map-rend-to-class(.)), $content)                            => model:map($node, $trackIds)
                        else
                            if (ancestor::teiHeader) then
                                ...
                                if (parent::person) then
                                    ...
                                    if (ancestor::text) then
                                        html:inline($config, ., ("tei-persName4", css:map-rend-to-class(.)), .)                                        => model:map($node, $trackIds)
                                    else
                                        $config?apply($config, ./node())
                    ...
                    case element(person) return
                        if (parent::listPerson) then
                            html:block($config, ., ("tei-person1", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                        else
                            if ($parameters?display='commentary') then
                                let $params := 
                                    map {
                                ...
                                return
                                                                html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-person2", "person-entry", css:map-rend-to-class(.)), $content)                                => model:map($node, $trackIds)
                            else
                    ...
                    case element(institution) return
                        html:block($config, ., ("tei-institution", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    case element(collection) return
                        html:block($config, ., ("tei-collection", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    case element(repository) return
                        html:block($config, ., ("tei-repository", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    case element(idno) return
                        if (@type="shelfmark") then
                            ...
                            if (@type="URLCatalogue") then
                                html:link($config, ., ("tei-idno2", css:map-rend-to-class(.)), ., (), (), map {})                                => model:map($node, $trackIds)
                            else
                                if (@type="URLImages") then
                                    html:link($config, ., ("tei-idno3", css:map-rend-to-class(.)), ., (), (), map {})                                    => model:map($node, $trackIds)
                                else
                                    if (@type="uri") then
                                        html:link($config, ., ("tei-idno4", css:map-rend-to-class(.)), ., ., (), map {})                                        => model:map($node, $trackIds)
                                    else
                                        $config?apply($config, ./node())
                    case element(placeName) return
                        if (ancestor::profileDesc and matches(., 'N/A')) then
                            ...
                            return
                                                        html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-placeName1", css:map-rend-to-class(.)), $content)                            => model:map($node, $trackIds)
                        else
                            if (ancestor::teiHeader) then
                    ...
                    case element(editionStmt) return
                        if ($parameters?display='commentary') then
                            ...
                            return
                                                        html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-editionStmt", css:map-rend-to-class(.)), $content)                            => model:map($node, $trackIds)
                        else
                            $config?apply($config, ./node())
                    case element(space) return
                        if (@quantity) then
                            html:inline($config, ., ("tei-space", css:map-rend-to-class(.)), @quantity)                            => model:map($node, $trackIds)
                        else
                            $config?apply($config, ./node())
                    case element(metamark) return
                        html:inline($config, ., ("tei-metamark", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    case element(affiliation) return
                        if (parent::person) then
                            html:block($config, ., ("tei-affiliation", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                        else
                            $config?apply($config, ./node())
                    case element(occupation) return
                        if (parent::person) then
                            html:block($config, ., ("tei-occupation", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                        else
                            $config?apply($config, ./node())
                    case element(org) return
                        if (parent::listOrg) then
                            html:block($config, ., ("tei-org", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                        else
                            $config?apply($config, ./node())
                    case element(orgName) return
                        if (parent::org) then
                            html:block($config, ., ("tei-orgName", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                        else
                            $config?apply($config, ./node())
                    case element(msName) return
                        html:inline($config, ., ("tei-msName", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
