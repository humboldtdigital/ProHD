...
module namespace model="http://www.tei-c.org/pm/models/prohd/epub";
            ...
            return
                    ...
                    case element(teiHeader) return
                        if ($parameters?header='short') then
                            ...
                            if ($parameters?display='commentary') then
                                ...
                                return
                                                                epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-teiHeader4", css:map-rend-to-class(.)), $content)
                            else
                    ...
                    case element(choice) return
                        if (abbr and expan) then
                            (
                                epub:alternate($config, ., ("tei-choice1", "choice", css:map-rend-to-class(.)), ., expan[1], abbr[1]),
                                epub:alternate($config, ., ("tei-choice2", "choice-alternate", css:map-rend-to-class(.)), ., abbr[1], expan[1])
                            )

                        else
                            if (sic and corr) then
                                (
                                    epub:alternate($config, ., ("tei-choice3", "choice", css:map-rend-to-class(.)), ., corr[1], sic[1]),
                                    epub:alternate($config, ., ("tei-choice4", "choice-alternate", css:map-rend-to-class(.)), ., sic[1], corr[1])
                                )

                            else
                                if (reg and orig) then
                                    (
                                        epub:alternate($config, ., ("tei-choice5", "choice", css:map-rend-to-class(.)), ., reg[1], orig[1]),
                                        epub:alternate($config, ., ("tei-choice6", "choice-alternate", css:map-rend-to-class(.)), ., orig[1], reg[1])
                                    )

                                else
                                    if ($parameters?mode='norm' and abbr and expan) then
                                        epub:alternate($config, ., ("tei-choice7", "choice", css:map-rend-to-class(.)), ., expan[1], abbr[1])
                                    else
                                        if ($parameters?mode='norm' and sic and corr) then
                                            epub:alternate($config, ., ("tei-choice8", "choice", css:map-rend-to-class(.)), ., corr[1], sic[1])
                                        else
                                            if ($parameters?mode='norm' and orig and reg) then
                                                epub:alternate($config, ., ("tei-choice9", "choice", css:map-rend-to-class(.)), ., reg[1], orig[1])
                                            else
                                                $config?apply($config, ./node())
                    case element(hi) return
                        if (@rendition="#u" and @hand="#unknown") then
                            (
                                ...
                                return
                                                                epub:note(map:merge(($config, map:entry("template", true()))), ., ("tei-hi2", css:map-rend-to-class(.)), $content, (), ())
                            )
                                ...
                                if (parent::person or parent::place) then
                                    ...
                                    if (@place) then
                                        ...
                                        if (parent::div and not(@place)) then
                                            epub:block($config, ., ("tei-note5", css:map-rend-to-class(.)), .)
                                        else
                                            if (not(@place)) then
                    ...
                    case element(dateline) return
                        if (@rend="align(right)") then
                            epub:block($config, ., ("tei-dateline1", css:map-rend-to-class(.)), .)
                        else
                            if (not(@rend)) then
                                epub:block($config, ., ("tei-dateline2", css:map-rend-to-class(.)), .)
                            else
                                $config?apply($config, ./node())
                    ...
                    case element(subst) return
                        if ($parameters?mode='norm' and del and add) then
                            epub:alternate($config, ., ("tei-subst", "choice", css:map-rend-to-class(.)), ., add[1], del[1])
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(seg) return
                        if (@type="comment") then
                            (
                                ...
                                return
                                                                epub:note(map:merge(($config, map:entry("template", true()))), ., ("tei-seg2", css:map-rend-to-class(.)), $content, (), ())
                            )
                    ...
                    case element(profileDesc) return
                        if ($parameters?header='short') then
                            (
                                epub:block($config, ., ("tei-profileDesc1", "header-short", css:map-rend-to-class(.)), abstract[@xml:lang="es"])
                            )
                            ...
                            if ($parameters?display='commentary' and creation[count(orgName)=2]) then
                                ...
                                return
                                                                epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-profileDesc2", css:map-rend-to-class(.)), $content)
                            else
                                if ($parameters?display='commentary' and creation[count(orgName)=1]) then
                                    ...
                                    return
                                                                        epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-profileDesc3", css:map-rend-to-class(.)), $content)
                                else
                                    if ($parameters?display='commentary' and correspDesc) then
                                        ...
                                        return
                                                                                epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-profileDesc4", css:map-rend-to-class(.)), $content)
                                    else
                                        if ($parameters?display='commentary' and creation[count(persName)=2]) then
                                            ...
                                            return
                                                                                        epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-profileDesc5", css:map-rend-to-class(.)), $content)
                                        else
                                            if ($parameters?display='commentary' and creation[count(persName)=1]) then
                                                ...
                                                return
                                                                                                epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-profileDesc6", css:map-rend-to-class(.)), $content)
                                            else
                    ...
                    case element(date) return
                        if (ancestor::profileDesc and matches(., 'N/A')) then
                            ...
                            if (ancestor::text and @when) then
                                epub:alternate($config, ., ("tei-date4", css:map-rend-to-class(.)), ., ., @when)
                            else
                                if (text()) then
                    ...
                    case element(titleStmt) return
                        if ($parameters?header='short') then
                            (
                                ...
                                return
                                                                epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-titleStmt4", css:map-rend-to-class(.)), $content),
                                html:inline($config, ., ("tei-titleStmt5", css:map-rend-to-class(.)), author),
                                (: incipit :)
                                epub:block($config, ., ("tei-titleStmt6", "incipit", css:map-rend-to-class(.)), root(.)//seg[@type='incipit'])
                            )
                        ...
                        if (@type='folNum' and $parameters?view='page') then
                            ...
                            if (@place='top' and $parameters?view='page') then
                                ...
                                if (@type='sig' and $parameters?view='page') then
                                    epub:block($config, ., ("tei-fw3", css:map-rend-to-class(.)), .)
                                else
                                    if (@type='catch' and $parameters?view='page') then
                                        epub:block($config, ., ("tei-fw4", css:map-rend-to-class(.)), .)
                                    else
                    ...
                    case element(person) return
                        if (parent::listPerson) then
                            epub:block($config, ., ("tei-person1", css:map-rend-to-class(.)), .)
                        else
                            if ($parameters?display='commentary') then
                                let $params := 
                                    map {
                    ...
                    case element(institution) return
                        epub:block($config, ., ("tei-institution", css:map-rend-to-class(.)), .)
                    case element(collection) return
                        epub:block($config, ., ("tei-collection", css:map-rend-to-class(.)), .)
                    case element(repository) return
                        epub:block($config, ., ("tei-repository", css:map-rend-to-class(.)), .)
                    ...
                    case element(editionStmt) return
                        if ($parameters?display='commentary') then
                            ...
                            return
                                                        epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-editionStmt", css:map-rend-to-class(.)), $content)
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(affiliation) return
                        if (parent::person) then
                            epub:block($config, ., ("tei-affiliation", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    case element(occupation) return
                        if (parent::person) then
                            epub:block($config, ., ("tei-occupation", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    case element(org) return
                        if (parent::listOrg) then
                            epub:block($config, ., ("tei-org", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    case element(orgName) return
                        if (parent::org) then
                            epub:block($config, ., ("tei-orgName", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
