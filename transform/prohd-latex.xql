            ...
            return
                    ...
                    case element(pb) return
                        if (@facs) then
                            ...
                            return
                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-pb1", "lb", css:map-rend-to-class(.)), $content)
                        else
                            if (@facs) then
                                (: No function found for behavior: webcomponent :)
                                $config?apply($config, ./node())
                    ...
                    case element(choice) return
                        if (abbr and expan) then
                            (
                                latex:alternate($config, ., ("tei-choice1", "choice", css:map-rend-to-class(.)), ., expan[1], abbr[1]),
                                latex:alternate($config, ., ("tei-choice2", "choice-alternate", css:map-rend-to-class(.)), ., abbr[1], expan[1])
                            )

                        else
                            if (sic and corr) then
                                (
                                    latex:alternate($config, ., ("tei-choice3", "choice", css:map-rend-to-class(.)), ., corr[1], sic[1]),
                                    latex:alternate($config, ., ("tei-choice4", "choice-alternate", css:map-rend-to-class(.)), ., sic[1], corr[1])
                                )

                            else
                                if (reg and orig) then
                                    (
                                        latex:alternate($config, ., ("tei-choice5", "choice", css:map-rend-to-class(.)), ., reg[1], orig[1]),
                                        latex:alternate($config, ., ("tei-choice6", "choice-alternate", css:map-rend-to-class(.)), ., orig[1], reg[1])
                                    )
                                    ...
                                    if ($parameters?mode='norm' and abbr and expan) then
                                        ...
                                        if ($parameters?mode='norm' and sic and corr) then
                                            latex:alternate($config, ., ("tei-choice8", "choice", css:map-rend-to-class(.)), ., corr[1], sic[1])
                                        else
                                            if ($parameters?mode='norm' and orig and reg) then
                                                latex:alternate($config, ., ("tei-choice9", "choice", css:map-rend-to-class(.)), ., reg[1], orig[1])
                                            else
                                                $config?apply($config, ./node())
                    ...
                    case element(text) return
                        if ($parameters?display='landing') then
                            latex:body($config, ., ("tei-text1", css:map-rend-to-class(.)), let $lang := $parameters?language let $l := if ($lang = ('en', 'es', 'de')) then $lang else 'en' return root(.)//text[@xml:lang=$l])
                        else
                            latex:body($config, ., ("tei-text2", css:map-rend-to-class(.)), .)
